...
export function CopyIp({ ip }: { ip: string }) {
  ...
  return (
    <button
      ...
      className="group inline-flex items-center gap-3 px-5 py-3 rounded-lg bg-card/70 border border-border hover:border-primary/40 transition-colors"
    >
      <span className="text-xs uppercase tracking-widest text-muted-foreground">Server IP</span>
      <span className="font-mono text-base md:text-lg text-primary">{ip}</span>
