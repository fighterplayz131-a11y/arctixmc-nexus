...
export const Route = createFileRoute("/cart")({
  head: () => ({
    meta: [
      { title: "Cart — ArctixMC" },
      { name: "description", content: "Review your ArctixMC store items and checkout." },
    ],
  }),
  component: CartPage,
});
...
function CartPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 md:px-8 py-12 animate-fade-in">
      <h1 className="font-display text-4xl md:text-5xl font-bold gradient-text mb-8">Your Cart</h1>
      ...
      {cart.length === 0 ? (
        <div className="rounded-2xl glass-strong p-12 text-center">
          ...
          <Link to="/store">
            <Button className="gradient-primary text-primary-foreground glow-primary">Browse Store</Button>
          </Link>
        </div>
      ) : (
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-3">
            {cart.map((item) => (
              <div key={item.id} className="rounded-xl glass-strong p-4 flex items-center gap-4">
                <div className="h-12 w-12 rounded-lg gradient-primary flex items-center justify-center font-display font-bold text-primary-foreground shrink-0">
                  {item.type === "rank" ? "R" : item.type === "coins" ? "C" : "K"}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-semibold truncate">{item.name}</div>
                  <div className="text-sm text-muted-foreground">रु {item.price} each</div>
                </div>
                ...
                <div className="font-bold gradient-text w-24 text-right">रु {item.price * item.quantity}</div>
          ...
          <div className="rounded-2xl glass-strong p-6 h-fit sticky top-24">
            ...
            <div className="border-t border-border pt-4 flex justify-between items-baseline mb-5">
              ...
              <span className="font-display text-2xl font-bold gradient-text">रु {total}</span>
            </div>
            <Button onClick={handleCheckout} className="w-full gradient-primary text-primary-foreground glow-primary h-11">
      ...
      <Dialog open={checkoutOpen} onOpenChange={setCheckoutOpen}>
        <DialogContent className="glass-strong border-primary/20">
          ...
          <div className="space-y-3">
            <div className="rounded-lg glass p-3 flex justify-between">
            ...
            <div className="flex justify-between items-baseline px-1">
              <span className="font-semibold">Total</span>
              <span className="font-display text-xl font-bold gradient-text">रु {total}</span>
            </div>
          </div>
          <DialogFooter>
